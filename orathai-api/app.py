...
app = Flask(__name__)
#@app.route('/')
@app.route('/orathai/<word>',methods=["GET"])
...
def getData(word):
	try:
		...
		def callPhlongTaIam(sent):
			...
			if sent != "":
				# url = "http://wordcut.krooklon.com/wordcut/wcut.php?string="
				url = "https://wordcut-api.herokuapp.com/wcut.php?string="
		...
		def KamKom(sent):
			# print (sent)
			data = {}
			with open('KK.json', encoding='UTF-8') as json_data:
				data = json.load(json_data)
				
			prob = {}

			with open("prob.txt", encoding='UTF-8') as f:
				for line in f:
		...
		def readData(word):
			...
			payang = []
			wak = []
			with open('words.json', encoding='UTF-8') as json_data:
				data = json.load(json_data)
